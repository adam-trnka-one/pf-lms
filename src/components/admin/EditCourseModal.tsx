...
import type { Course, CourseFormData, CourseChapter } from '../../types/course';
...
interface EditCourseModalProps {
  course: Course;
  onClose: () => void;
  onSave: (courseId: string, updates: Partial<CourseFormData>) => Promise<void>;
}
...
export default function EditCourseModal({ course, onClose, onSave }: EditCourseModalProps) {
  ...
  const [formData, setFormData] = useState<CourseFormData>({
    ...
    certificateValidity: {
      months: course.certificateValidity.months,
      renewable: course.certificateValidity.renewable,
      requiresAssessment: course.certificateValidity.requiresAssessment
    },
  ...
  const handleFormChange = (updates: Partial<CourseFormData>) => {
      ...
      <div className="relative top-10 mx-auto max-w-4xl">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div 
            ...
            style={{
              backgroundImage: `linear-gradient(to right, rgba(255, 117, 29, 0.9), rgba(255, 178, 133, 0.9)), url(${formData.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(formData.title)}&background=ff751d&color=fff&size=400`})`
            }}
          >
            <button
              onClick={handleClose}
          ...
          <form onSubmit={handleSubmit} className="pt-8 pb-8 px-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-1">Edit Course</h2>
            <p className="text-sm text-gray-500 mb-8">Update course information and settings</p>
                ...
                <div className="mb-4">
                  <label className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-700">
                      I confirm that all the information provided is correct and I want to save these changes
                    </span>
                  </label>
                </div>
                <div className="flex justify-end space-x-3">
